...
以上为寄存器保存与恢复的汇编代码，接下来我们对这段代码做一下分析。
...
图中的红色线条表示将要切出的协程对象加入接续队列尾部，而绿色线条表示要将队列头部的协程对象取出作为当前运行协程。
...
这里的`yield_coroutine`是指选择一个协程切换。所以这里的逻辑就是，当前协程什么都不做，就只是切换到其他协程，直到要等待的协程状态达到`finished`状态。
...
协程合并是指将指定协程合并到本地协程。单纯这么描述可能比较抽象，使用标准库的线程来说就是`join`函数。`join`具有等待指定协程执行结束并回收资源的作用。

#### 协程分离

协程分离是指不再管控当前协程。在调用协程分离函数（`detach`）之后，被操作的协程将不再受任何协程管控，对应的任何协程用户界面对象都将会失效，对这些失效的对象进行任何操作都会出现错误。

协程分离如何实现呢？可以对被分离的协程对象做出标记，当这些带有分离标记的协程执行结束时，直接释放其资源就可以了。

而还有一种可能，在调用协程分离函数之前，协程就已经结束了。所以应该在调用协程分离函数的时候判断一下协程状态，如果已经结束，就直接销毁资源，否则设置分离标记即可。

#### 协程对象销毁

上面讨论了两种协程的“归宿”，一种是合并，一种是分离。任何一个协程，最终要么被合并到其他协程，要么分离出来独自运行。而无论是合并还是分离，最后都会执行协程对象的销毁。

协程对象的销毁可以参照协程对象的创建，创建了哪些对象就销毁哪些对象，比如，对于共享栈协程的协程对象，就不能销毁其共享栈，因为这个空间是由协程运行环境对象申请的。

#### 协程运行环境销毁

在某些情况下可能需要对运行环境进行销毁，如进程需要退出，或者协程运行环境超过基准线程数量而又没有多余的协程可供执行时。

对于协程运行环境的退出，需要有以下步骤：

1. 等待当前运行环境上的所有协程运行结束
2. 关联的线程退出
3. 销毁资源

对于第1点，运行环境需要等待本环境中的协程都运行结束。这里需要注意的点是并发问题，当等待本环境中协程要退出的时候，有新的协程被分配到这个协程中怎么办？为了避免这种情况，我们可以将运行环境也增加对应的状态进行管理。比如状态有`NEW`、`WORKING`、`DESTROYING`，要销毁某个运行环境的时候，将状态设置为`DESTROYING`，此时新的协程就不会分配到执行环境上了。

第2点和第3点，在所有的协程都结束的时候，将运行环境从运行环境管理对象中移除，并销毁管理对象，然后将关联的线程进行合并即可。
